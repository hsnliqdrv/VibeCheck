...
2. Gets a presigned URL for avatar upload
...
def register_or_login():
    ...
    log("Step 1: Attempting to register/login user...")
...
def get_presigned_url(token: str, file_extension: str = "png"):
    ...
    response = requests.post(
        f"{BASE_URL}/upload/avatar?file_extension={file_extension}",
        headers=headers
    )
...
def main():
    """Main test execution"""
    log("Starting avatar upload test...", "INFO")
    print()
    ...
    if success:
        log("Avatar upload test completed successfully!", "SUCCESS")
        cdn_url = presigned_data.get('cdn_url', 'unknown')
        log(f"Avatar accessible at: {cdn_url}", "INFO")
    else:
        log("Avatar upload test failed", "ERROR")
        sys.exit(1)
