...
def get_badges():
    ...
    try:
        ...
        if badge_count == 0:
            print("⚠️ [/badges endpoint] No badges found in database, auto-seeding...")
            seed_badges(db)
            seed_curator_levels(db)
            badge_count = db.query(Badge).count()
            print(f"✓ [/badges endpoint] Badge count after seeding: {badge_count}")
        else:
            print(f"ℹ️ [/badges endpoint] Found {badge_count} existing badges in database")
